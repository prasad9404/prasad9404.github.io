...
var total_recover_data = JSON.parse(document.getElementById('data_india_total_recovered_data').textContent);
var total_death_data = JSON.parse(document.getElementById('data_india_total_deceased_data').textContent);
...
let x = []
for(let i=(total_confirm_data.length-20);i<total_confirm_data.length;i++)
{
  console.log(total_confirm_data[i],total_death_data[i],total_recover_data[i]);
  x.push(total_confirm_data[i]-(total_death_data[i]+total_recover_data[i]));
}
console.log(x);
window.Apex = {
    chart: {
